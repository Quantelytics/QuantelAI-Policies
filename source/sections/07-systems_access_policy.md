...
1. Requests for access to Quantel AI Platform systems and applications are made formally using the following process:
  ...
  4. If the review is approved, the Security Officer or Privacy Officer then marks the Issue as Done, adding any pertinent notes required. The Security Officer or Privacy Officer then grants requested access.
     * New accounts will be created with a temporary secure password that meets all requirements from [§7.11](#7.11-password-management), which must be changed on the initial login.
     ...
     * For non-production systems, access grants are accomplished by leveraging the access control mechanisms built into those systems. Account management for non-production systems may be delegated to a Quantel AI employee at the discretion of the Security Officer or Privacy Officer .
2. Access is not granted until receipt, review, and approval by Quantel AI Security Officer or Privacy Officer.
3. The request for access is retained for future reference.
4. All access to Quantel AI systems and services is reviewed and updated on a bi-annual basis to ensure proper authorizations are in place commensurate with job functions. The process for conducting reviews is outlined below:
   1. The Security Officer initiates the review of user access by creating an Issue in Quantel AI Quality Management System.
   2. The Security Officer is assigned to review levels of access for each Quantel AI workforce member.
   ...
   6. Review of user access is monitored on a quarterly basis using Quality Management System reporting to assess compliance with above policy.
5. Any Quantel AI workforce member can request change of access using the process outlined in [§7.1 paragraph 1](#7.1-access-establishment-and-modification).
...
8. In the case of non-personal information, such as generic educational content, identification and authentication may not be required. This is the responsibility of Quantel AI Customers to define, and not Quantel AI.
9. Privileged users must first access systems using standard, unique user accounts before switching to privileged users and performing privileged tasks.
   * For production systems, this is enforced by creating non-privileged user accounts that must invoke a 'break the glass' process to perform privileged tasks.
   * Rights for privileged accounts are granted by the Security Officer or Privacy Officer using the process outlined in [§7.1 paragraph 1](#7.1-access-establishment-and-modification).
...
11. Generic accounts are not allowed on Quantel AI systems.
...
3. Quantel AI maintains a minimum necessary approach to access to Customer data. As such, Quantel AI, including all workforce members, does not readily have access to any sensitive Customer data.

## 7.3 Access Authorization

1. Role based access categories for each Quantel AI system and application are pre-approved by the Security Officer, or an authorized delegate of the Security Officer.
2. Quantel AI utilizes hardware and software firewalls to segment data, prevent unauthorized access, and monitor traffic for denial of service attacks.
...
3. All Customer support desk interactions must be verified before Quantel AI support personnel will satisfy any request having information security implications.
   * Quantel AI's current support desk software, Zendesk, requires users to authenticate before submitting support tickets.
   * Support issues submitted via Quantel AI's dashboard require that users authenticate with their Quantel AI account before submitting support tickets.
   * Support issues submitted by email must be verified by Quantel AI personnel using a phone number that has been registered with the corresponding account.

## 7.5 Unique User Identification

1. Access to Quantel AI Platform systems and applications is controlled by requiring unique User Login IDs and passwords for each individual user and developer.
2. Passwords requirements mandate strong password controls (see below).
3. Passwords are not displayed at any time and are not transmitted or stored in plain text.
4. Default accounts on all production systems, including root, are disabled.
5. Shared accounts are not allowed within Quantel AI systems or networks.
6. Automated log-on configurations that store user passwords or bypass password entry are not permitted for use with Quantel AI workstations or production systems.
...
All workstations at Quantel AI are company owned, and all are laptop Apple products running Mac OSX or Linux.
...
9. All workstations are to have the following messages added to the lock screen and login screen: *This computer is owned by Quantel AI Health, Inc. By logging in, unlocking, and/or using this computer you acknowledge you have seen, and follow, these policies (https://policy.quantel.ai) and have completed this training (https://training.quantel.ai/). Please contact us if you have problems with this - privacy@quantel.ai.*

## 7.8 Wireless Access Use

1. Quantel AI production systems are not accessible directly over wireless channels.¶
...
4. Wireless networks managed within Quantel AI non-production facilities (offices, etc.) are secured with the following configurations:
   * All data in transit over wireless is encrypted using WPA2 encryption;
   * Passwords are rotated on a regular basis, presently quarterly. This process is managed by Quantel AI Security Officer.
...
3. The Security Officer will terminate users' access rights immediately upon notification, and will coordinate with the appropriate Quantel AI employees to terminate access to any non-production systems managed by those employees.
...
Quantel AI does not use paper records for any sensitive information. Use of paper for recording and storing sensitive data is against Quantel AI policies.

## 7.11 Password Management

1. User IDs and passwords are used to control access to Quantel AI systems and may not be disclosed to anyone for any reason.
2. Users may not allow anyone, for any reason, to have access to any information system using another user's unique user ID and password.
3. On all production systems and applications in Quantel AI environment, password configurations are set to require:
...
4. All system and application passwords must be stored and transmitted securely.
   ...
   * Passwords that must be stored in non-hashed format must be encrypted at rest pursuant to the requirements in [§17.7](#17.7-production-data-security).¶
...
6. Passwords are inactivated immediately upon an employee's termination (refer to the [Employee Termination Procedures in §7.9](#7.9-employee-termination-procedures)).
...
12. In cases where a user has forgotten their password, the following procedure is used to reset the password.
    * The user submits a password reset request to password-reset@quantel.ai. The request should include the system to which the user has lost access and needs the password reset.¶
...
## 7.12 Access to sensitive Customer data

1. Employees may not download sensitive Customer data to any workstations used to connect to production systems.
2. Disallowing transfer of sensitive Customer data to workstations is enforced through technical measures.
   * All production access to systems is performed through a bastion/jump host accessed through a VPN. Direct access to production systems is disallowed by Quantel AI's VPN configuration.
...
Quantel AI grants PaaS customer secure system access via VPN connections. This access is only to Customer-specific systems, no other systems in the environment. These connections are setup at customer deployment. These connections are secured and encrypted and the only method for customers to connect to Quantel AI hosted systems.

In the case of data migration, Quantel AI does, on a case by case basis, support customers in importing data. In these cases Quantel AI requires that all data is secured and encrypted in transit, such as by using SFTP or SCP for transferring files.

In the case of an investigation, Quantel AI will assist customers, at Quantel AI's discretion, and law enforcement in forensics.
